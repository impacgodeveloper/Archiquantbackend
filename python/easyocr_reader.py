    ...
    for i, (bbox, text, prob) in enumerate(raw_sorted):
        ...
        if i+1 < len(raw_sorted) and i+1 not in used:
            ...
            if abs(y2-y) < 25 and abs(x2-x) < 130 and prob2 >= MIN_CONF:
                ...
                if re.search(r'[WD].*\d+.*[Xx×].*\d+', merged_text):
...
def clean(t):
    ...
    # Fix OCR letter/digit confusion
...
# Remove legend/footer labels (bottom 20% = legend area, not actual drawing)
legend_y = img_h * 0.80
LEGEND_KEYWORDS = [
...
def parse_metric(val_str):
    try: return round(float(val_str) * 3.281, 1)
    except: return None
 
# ══════════════════════════════════════════════
#  DETECT DRAWING UNIT (metric or imperial)
# ══════════════════════════════════════════════
def detect_unit():
    ...
    for e in elements:
        ...
        if re.search(r"\d+[xX]\d+", t) and not re.search(r"\d+\.\d+", t): imperial_hits += 1
...
# Recompute total_walls_connected after dedup
...
# ]
# WINDOW_PATTERNS = [
#     # Standard: W=4x4, W=4X4
#     r"^W[=\-\s]?(\d+\.?\d*)[xX×](\d+\.?\d*)$",
#     # Feet format: W=4'x4', W=4'6"x4'
#     r"^W[=\-\s]?(\d+[''\']\d*[\"']?)\s*[xX×]\s*(\d+[''\']\d*[\"']?)$",
#     # Merged split: W-46"X4 or W=4'6"X4 (OCR merged token)
#     r"^W[=\-]?(\d+)'?(\d*)[\"']?[xX×](\d+)[''\"']?$",
#     # With label: W1 1.35x1.20
#     r"^W\d+[\s\n]?(\d+\.?\d*)[xX×](\d+\.?\d*)$",
#     r"^WIN[=\-\s]?(\d+\.?\d*)[xX×](\d+\.?\d*)$",
#     r"^WINDOW[=\-\s]?(\d+\.?\d*)[xX×](\d+\.?\d*)$",
#     r"^W\d+$", r"^WIN\d*$", r"^WINDOW\d*$", r"^FENETRE\d*$",
...
#     # Merge split detections on same line (e.g. "W-4" + "6\"X4" → "W=4\'6\"X4")
#     merged = []
#     used   = set()
#     raw_sorted = sorted(raw_results, key=lambda r: (round(r[0][0][1]/20), r[0][0][0]))
#     for i, (bbox, text, prob) in enumerate(raw_sorted):
...
#     t = re.sub(r'\b([TH])-(\d)', r'\1=\2', t)
#     t = re.sub(r'\b([DW])-(\d)', r'\1=\2', t)
#     # Fix OCR garbling: TZ4→T=4, H-L0→H=10, T-G→T=9
#     t = re.sub(r'\bTZ(\d)', r'T=\1', t)
#     t = re.sub(r'H-L(\d)', r'H=1\1', t)
#     t = t.replace("T-G","T=9").replace("T=G","T=9")
#     # Fix W-4 style (dash after W/D before digit)
#     t = re.sub(r'\bW-(\d)', r'W=\1', t)
#     t = re.sub(r'\bD-(\d)', r'D=\1', t)
...
# # ── Remove legend/footer EW/IW labels (bottom 20% of image = legend area) ──
# # Real wall labels are inside the drawing, not in the legend box at the bottom
# legend_y = img_h * 0.80 if raw_images else 99999
# elements = [e for e in elements if not (
#     e["y"] > legend_y and
#     any(kw in e["text"] for kw in ["= EXTERNAL WALL","= INTERNAL WALL","EXTERNAL WALL","EW =","IW =","EW:","ROOM TINTS","BEDROOMS","BATHROOMS"])
# )]
