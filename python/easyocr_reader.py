...
# ================= METRIC HELPERS =================
def ft_to_m(v):
    return round(v * 0.3048, 2) if v else None

def sqft_to_sqm(v):
    return round(v * 0.0929, 2) if v else None
...
def parse_size_from_elements(base):
    ...
    if best_match:
        w, l = best_match
        if w > l: w, l = l, w
        return {"length_ft": round_to_next_number(l), "width_ft": round_to_next_number(w)}
...
def nearest_room(base):
    ...
    for e in elements:
        ...
        if txt in ["NORTH","SOUTH","EAST","WEST"]: continue
        if re.match(size_pattern, txt): continue
        d = dist(base, e)
        if d < best_d:
            best = txt
            best_d = d
...
for (w, h), c in Counter(door_sizes).items():
    ...
    doors.append({
        ...
        "size_m":  {"width": ft_to_m(w), "height": ft_to_m(h)},
        "count": c,
        "area_each_sqft": area_each,
        "area_each_sqm":  sqft_to_sqm(area_each),
        "total_area_sqft": area_each * c,
        "total_area_sqm":  sqft_to_sqm(area_each * c),
    })
...
for e in elements:
    ...
    if "IW" in txt:
        ...
        if t and h and (3 <= t <= 12) and (6 <= h <= 12):
            iw_sizes.append((t, h)); visited.append(e)
    elif "EW" in txt:
        t, h = find_wall(e, "EW")
        if t and h and (6 <= t <= 15) and (8 <= h <= 15):
            ew_sizes.append((t, h)); visited.append(e)

internal_walls = []
external_walls = []
for (t, h), c in Counter(iw_sizes).items():
    internal_walls.append({
        "type": "internal", "thickness_inch": t, "height_ft": h,
        "thickness_m": ft_to_m(t/12), "height_m": ft_to_m(h), "count": c
    })
for (t, h), c in Counter(ew_sizes).items():
    external_walls.append({
        "type": "external", "thickness_inch": t, "height_ft": h,
        "thickness_m": ft_to_m(t/12), "height_m": ft_to_m(h), "count": c
    })
...
# ================= EW/IW ANCHORS (for zone connectivity) =================
ew_anchors = []
iw_anchors = []

for e in elements:
    txt = e["text"].replace("1W","IW").replace("lW","IW")
    if re.match(r"^EW[\s\d]*$|^EW\d+[A-Z]?$|^EW$", txt):
        ew_anchors.append(dict(e, id=f"ew{len(ew_anchors)+1}"))
    elif re.match(r"^IW[\s\d]*$|^IW\d+[A-Z]?$|^IW$|^IW[\s\(]", txt):
        iw_anchors.append(dict(e, id=f"iw{len(iw_anchors)+1}"))

# Add zone connectivity
for z in zones:
    base  = next((e for e in elements if e["text"] == z["zone_id"]), None)
    if not base:
        z["connected_external_walls"] = []
        z["connected_internal_walls"] = []
        z["total_walls_connected"]    = 0
        z["doors"]   = []
        z["windows"] = []
        continue

    area  = z["area_sqft"] or 100
    wft   = z["width_ft"]  or 10
    lft   = z["length_ft"] or 10
    # Proximity radius based on shorter side
    img_w = max((e["x"] for e in elements), default=1000)
    px_ft = img_w / 40.0
    zp    = min(wft, lft) * px_ft * 0.55

    conn_ew = list(dict.fromkeys(
        a["id"] for a in ew_anchors
        if math.hypot(base["x"]-a["x"], base["y"]-a["y"]) < zp * 1.3
    ))
    conn_iw = list(dict.fromkeys(
        a["id"] for a in iw_anchors
        if math.hypot(base["x"]-a["x"], base["y"]-a["y"]) < zp * 1.1
    ))
    conn_d = list(dict.fromkeys(
        f"d{i+1}" for i,(x,y) in enumerate(door_positions)
        if math.hypot(base["x"]-x, base["y"]-y) < zp
    ))
    conn_w = list(dict.fromkeys(
        f"w{i+1}" for i,(x,y) in enumerate(window_positions)
        if math.hypot(base["x"]-x, base["y"]-y) < zp
    ))

    z["connected_external_walls"] = conn_ew
    z["connected_internal_walls"] = conn_iw
    z["total_walls_connected"]    = len(conn_ew) + len(conn_iw)
    z["doors"]   = conn_d
    z["windows"] = conn_w
...
final = {
    ...
    "summary": {
        ...
        "total_opening_area_sqm":  sqft_to_sqm(total_door_area + total_window_area),
    }
}
